...
if torch_version_at_least("2.7.0") and has_triton():
    ...
    # =========================================================================
    # Pass 2: flat per-group quantize with fused nan_to_num + inline scale
    # =========================================================================
    @triton.autotune(configs=_configs, key=["numel"])
    @triton.jit
    def _fp8_per_group_quantize_kernel(
        input_ptr,
        out_ptr,
        group_amax_ptr,  # (N_GROUPS,) float32 amaxes from pass 1
        scales_out_ptr,  # (N_GROUPS,) float32 scales output (written once per group)
        offsets_ptr,     # (N_GROUPS,) int32 group end offsets
        numel: int,
        K: tl.int64,
        N_GROUPS: tl.int64,
        ...
        """
        Each block computes the scale inline from the group's amax (avoiding
        ~7 separate PyTorch kernel launches on tiny vectors), then applies
        fused nan_to_num + scale + clamp + FP8 cast to BLOCK_SIZE elements.
        """
        pid = tl.program_id(0)
        offs = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
        mask = offs < numel
        ...
        # Compute scale inline from amax (fuses ~7 PyTorch ops into register math).
        amax = tl.load(group_amax_ptr + group_id).to(tl.float32)
        scale = fp8_max / tl.maximum(amax, EPS)
        if ROUND_POW2:
            scale = tl.exp2(tl.floor(tl.log2(scale)))

        # Write scale for the caller. Multiple blocks in the same group
        # compute identical values, so redundant writes are safe.
        tl.store(scales_out_ptr + group_id, scale)
    ...
    def triton_fp8_tensorwise_per_group_quantize(
        ...
        GPU launches: amax kernel + quantize kernel (with inline scale
        computation) + scale expand = 3 total (vs ~17 in the PyTorch fallback).
        ...
        n_groups = offs.numel()
        assert n_groups <= 32, (
            f"n_groups={n_groups} exceeds kernel MAX_GROUPS=32"
        )
        ...
        # --- Pass 2: quantize (scale computed inline from amax) ---
        scales = torch.empty(n_groups, dtype=torch.float32, device=tensor.device)
