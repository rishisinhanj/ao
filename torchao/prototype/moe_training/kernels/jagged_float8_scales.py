...
if torch_version_at_least("2.7.0") and has_triton():
    ...
    # --- Kernel 1: Per-column-block amax (tensorwise) ---
    ...
    # --- Kernel 2: Quantize with per-group scale (inline from amax) ---
    # Grid: (ceil(K/BLOCK_SIZE), num_groups) — same parallelism as amax.
    # Each block computes scale from the group's amax, iterates over rows, quantizes.
    @triton.autotune(configs=kernel_configs_2D, key=["K", "N_GROUPS"])
    @triton.jit
    def _triton_fp8_tensorwise_quantize_kernel(
        input_ptr,
        offsets_ptr,
        group_amax_ptr,  # (N_GROUPS,) float32 amaxes from kernel 1
        scales_out_ptr,  # (N_GROUPS,) float32 computed scales (output)
        out_ptr,
        M: tl.int64,
        K: tl.int64,
        N_GROUPS: tl.int64,
        ...
        amax = tl.load(group_amax_ptr + group_idx).to(tl.float32)
        scale = fp8_dtype_max / tl.maximum(amax, EPS)
        if ROUND_POW2:
            scale = tl.exp2(tl.floor(tl.log2(scale)))

        if block_col_id == 0:
            tl.store(scales_out_ptr + group_idx, scale)
    ...
    def triton_fp8_per_group_tensorwise_scales(
        ...
        """
        Per-group tensorwise FP8 quantization using 2D tiled Triton kernels.

        Grid: (ceil(K/BLOCK_SIZE), num_groups) for both amax and quantize passes.
        No atomic_max, no offset scanning, proper 2D tiling for cache efficiency.
        ...
        scales_buffer = torch.empty(n_groups, dtype=torch.float32, device=hp_tensor.device)
    ...
    @torch.library.custom_op(
        "torchao::triton_fp8_per_group_tensorwise_scales_col_major",
        mutates_args={},
    )
    def triton_fp8_per_group_tensorwise_scales_col_major(
        ...
        """
        Same as triton_fp8_per_group_tensorwise_scales but writes FP8 output
        directly in column-major layout, avoiding a separate .copy_() transpose.

        Returns:
            - float8 tensor of shape (M, K) in column-major layout (strides: 1, M)
            - per-group tensorwise scales of shape (num_groups,)
        """
        assert hp_tensor.ndim == 2, "input tensor must be 2D"
        ...
        # Allocate col-major output: empty (K, M) then .t() gives (M, K) with strides (1, M).
        output_buffer = torch.empty(
            (k, m), dtype=output_dtype, device=hp_tensor.device
        ).t()
        ...
        amax_buffer = partial_amax_buffer.max(dim=0).values
    ...
    @triton_fp8_per_group_tensorwise_scales_col_major.register_fake
    def _fake_triton_fp8_per_group_tensorwise_scales_col_major(
        hp_tensor: torch.Tensor,
        offsets: torch.Tensor,
        output_dtype: torch.dtype = torch.float8_e4m3fn,
        round_scales_to_power_of_2: bool = False,
    ...
    # =========================================================================
    # Dual tensorwise per-group quantize (col-major output)
    # =========================================================================

    @triton.autotune(configs=kernel_configs_2D, key=["K", "N_GROUPS"])
    @triton.jit
    def _triton_fp8_tensorwise_amax_dual_kernel(
        input_ptr_1,
        stride_input_row_1: tl.int64,
        ...
        K2: tl.int64,
        offsets_ptr,
        partial_amax_ptr_1,  # (N_COL_BLOCKS, N_GROUPS)
        partial_amax_ptr_2,  # (N_COL_BLOCKS, N_GROUPS)
        M: tl.int64,
        K: tl.int64,
        N_GROUPS: tl.int64,
        N_COL_BLOCKS: tl.int64,
        ...
        """Dual amax: processes both tensors in the same grid launch."""
        block_col_id = tl.program_id(axis=0)
        group_idx = tl.program_id(axis=1)
        ...
        process_1 = block_col_id < tl.cdiv(K1, BLOCK_SIZE)
        process_2 = block_col_id < tl.cdiv(K2, BLOCK_SIZE)

        amax_1 = tl.zeros((BLOCK_SIZE,), dtype=tl.float32)
        amax_2 = tl.zeros((BLOCK_SIZE,), dtype=tl.float32)

        for row_start in range(
            group_row_start, group_row_end, BLOCK_SIZE_ITER
        ):
            row_offs = (row_start + tl.arange(0, BLOCK_SIZE_ITER)).to(tl.int64)
            row_mask = row_offs < group_row_end

            if process_1:
                offs_1 = (
                    row_offs[:, None] * stride_input_row_1
                    + block_col_offs[None, :] * stride_input_col_1
                )
            ...
            if process_2:
                offs_2 = (
                    row_offs[:, None] * stride_input_row_2
                    + block_col_offs[None, :] * stride_input_col_2
                )
        ...
        if process_1:
            scalar_1 = tl.max(amax_1, axis=0).to(tl.float32)
            tl.store(
                partial_amax_ptr_1 + block_col_id * N_GROUPS + group_idx,
                scalar_1,
            )
        if process_2:
            scalar_2 = tl.max(amax_2, axis=0).to(tl.float32)
            tl.store(
                partial_amax_ptr_2 + block_col_id * N_GROUPS + group_idx,
                scalar_2,
            )

    @triton.autotune(configs=kernel_configs_2D, key=["K", "N_GROUPS"])
    @triton.jit
    def _triton_fp8_tensorwise_quantize_dual_kernel(
        input_ptr_1,
        out_ptr_1,
        stride_input_row_1: tl.int64,
        stride_input_col_1: tl.int64,
        ...
        """Dual quantize: processes both tensors with col-major output."""
        block_col_id = tl.program_id(axis=0)
        group_idx = tl.program_id(axis=1)
        ...
        scale_1 = tl.zeros([], dtype=tl.float32)
        scale_2 = tl.zeros([], dtype=tl.float32)

        if process_1:
            a1 = tl.load(amax_ptr_1 + group_idx).to(tl.float32)
            scale_1 = fp8_dtype_max / tl.maximum(a1, EPS)
            if ROUND_POW2:
                scale_1 = tl.exp2(tl.floor(tl.log2(scale_1)))
            if block_col_id == 0:
                tl.store(scales_out_ptr_1 + group_idx, scale_1)

        if process_2:
            a2 = tl.load(amax_ptr_2 + group_idx).to(tl.float32)
            scale_2 = fp8_dtype_max / tl.maximum(a2, EPS)
            if ROUND_POW2:
                scale_2 = tl.exp2(tl.floor(tl.log2(scale_2)))
            if block_col_id == 0:
                tl.store(scales_out_ptr_2 + group_idx, scale_2)
        ...
        for row_start in range(
            ...
            if process_1:
                ...
                out_offs_1 = (
                    row_offs[:, None] * stride_output_row_1
                    + block_col_offs[None, :] * stride_output_col_1
                )
                tl.store(out_ptr_1 + out_offs_1, fp8_1, mask=mask_1)
            ...
            if process_2:
                ...
                out_offs_2 = (
                    row_offs[:, None] * stride_output_row_2
                    + block_col_offs[None, :] * stride_output_col_2
                )
                tl.store(out_ptr_2 + out_offs_2, fp8_2, mask=mask_2)

    @torch.library.custom_op(
        "torchao::triton_fp8_per_group_tensorwise_dual_col_major",
        mutates_args={},
    )
    def triton_fp8_per_group_tensorwise_dual_col_major(
        ...
        """
        Fused dual per-group tensorwise quantize with col-major output.

        Quantizes two tensors sharing the same M dimension and group offsets
        in a single pair of kernel launches (amax + quantize). Both outputs
        are written directly in column-major layout.

        Returns:
            - fp8_data_1: (M, K1) col-major FP8
            - scales_1: (n_groups,) float32
            - fp8_data_2: (M, K2) col-major FP8
            - scales_2: (n_groups,) float32
        """
        assert hp_tensor_1.ndim == 2 and hp_tensor_2.ndim == 2
        assert hp_tensor_1.shape[0] == hp_tensor_2.shape[0]
        ...
        k2 = hp_tensor_2.shape[1]
        k_max = max(k1, k2)
        n_groups = offsets.numel()

        out_1 = torch.empty(k1, m, dtype=output_dtype, device=hp_tensor_1.device).t()
        out_2 = torch.empty(k2, m, dtype=output_dtype, device=hp_tensor_2.device).t()

        min_block_size = min(c.kwargs["BLOCK_SIZE"] for c in kernel_configs_2D)
        n_col_blocks = triton.cdiv(k_max, min_block_size)
        pa_1 = torch.zeros((n_col_blocks, n_groups), dtype=torch.float32, device=hp_tensor_1.device)
        pa_2 = torch.zeros((n_col_blocks, n_groups), dtype=torch.float32, device=hp_tensor_2.device)

        amax_grid = lambda meta: (
            max(triton.cdiv(k1, meta["BLOCK_SIZE"]), triton.cdiv(k2, meta["BLOCK_SIZE"])),
            n_groups,
        )
        _triton_fp8_tensorwise_amax_dual_kernel[amax_grid](
            hp_tensor_1,
            hp_tensor_1.stride(0),
        ...
        amax_1 = pa_1.max(dim=0).values
        amax_2 = pa_2.max(dim=0).values
        scales_1 = torch.empty(n_groups, dtype=torch.float32, device=hp_tensor_1.device)
        scales_2 = torch.empty(n_groups, dtype=torch.float32, device=hp_tensor_2.device)

        quant_grid = lambda meta: (
            max(triton.cdiv(k1, meta["BLOCK_SIZE"]), triton.cdiv(k2, meta["BLOCK_SIZE"])),
            n_groups,
        )
        _triton_fp8_tensorwise_quantize_dual_kernel[quant_grid](
            hp_tensor_1,
            out_1,
            hp_tensor_1.stride(0),
            hp_tensor_1.stride(1),
        ...
        return out_1, scales_1, out_2, scales_2

    @triton_fp8_per_group_tensorwise_dual_col_major.register_fake
    def _fake_triton_fp8_per_group_tensorwise_dual_col_major(
        hp_tensor_1: torch.Tensor,
        hp_tensor_2: torch.Tensor,
        offsets: torch.Tensor,
        output_dtype: torch.dtype = torch.float8_e4m3fn,
        ...
        m, k1 = hp_tensor_1.shape
        _, k2 = hp_tensor_2.shape
        n_groups = offsets.numel()
        out_1 = torch.empty(k1, m, dtype=output_dtype, device=hp_tensor_1.device).t()
        out_2 = torch.empty(k2, m, dtype=output_dtype, device=hp_tensor_2.device).t()
        s1 = torch.empty(n_groups, dtype=torch.float32, device=hp_tensor_1.device)
        s2 = torch.empty(n_groups, dtype=torch.float32, device=hp_tensor_2.device)
        return out_1, s1, out_2, s2
